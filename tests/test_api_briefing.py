...
def test_without_raw_context_removes_only_raw_context():
    briefing = {
...
def test_get_counts_uses_permission_gate_and_timestamp():
...
def test_get_ceo_briefing_omits_raw_context_from_briefing():
    provider = MagicMock()
    ...
    assert result["data"] == _SAMPLE_CONTEXT
    assert "raw_context" not in result["briefing"]
    assert result["provider"]["external_call"] is False
    assert result["provider_response"]["summary"] == "provider summary"
